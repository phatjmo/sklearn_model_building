...
# xlsx_file = pd.ExcelFile('./SMS_to_Leases_09252017.xlsx')
# data = xlsx_file.parse("Export Worksheet")
data = pd.read_excel(open('./SMS_to_Leases_09252017.xlsx','rb'),
...
#     if rec == 1:
#         lease_count+=1
# print "Leases: %d" % lease_count
print y.value_counts()
# print data.groupby('turnedintolease').count()
# Drop data we can't use with this regression model. Better textual analysis is required
print "Step 4.1: Drop unnecessary fields."
X = data.drop(['turnedintolease', 'sitecustomerid'],
                axis=1)
# Testing Vectorizers
...
print "Step 4.2: Vectorize Conversations"
X['Conversation'] = vectorizer.fit_transform(X['Conversation'].values.astype('U')).toarray()
# print X['Comments']
...
print "Step 4.3: Split the damned data!"
